...
@router.get("/{article_id}", response_model=ArticleGetSchema)
...
@router.post("/", response_model=ArticleBaseSchema, status_code=status.HTTP_201_CREATED)
...
@router.put("/{article_id}", response_model=ArticleBaseSchema, status_code=status.HTTP_200_OK)
...
@router.delete("/hard/{article_id}", status_code=status.HTTP_204_NO_CONTENT)
...
@router.delete("/soft/{article_id}", status_code=status.HTTP_204_NO_CONTENT)
async def soft_delete_article(
    article_service: Annotated[ArticleService, Depends(get_article_service)],
    article_id: int
):
    return await article_service.soft_delete_article(article_id)
